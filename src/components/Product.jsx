  ...
  return (
    <div className={type==="similar"?'flex justify-center flex-col gap-3 items-center my-4 w-60':'flex justify-center flex-col items-center w-60 my-4 mx-2'}>
      <div className="relative w-full">
        <img className='object-cover w-full h-60' src={`${bf}/${product.image}`} alt="" loading='lazy'/>
      ...
      <Link to={product.type==="gifts" ? `/${product.type}/${product.category}/${product.id}` :`/product/${product.type}/${product.category}/${product.id}`}><p className='text-bold text-base text-center'>{product.title}</p></Link>
